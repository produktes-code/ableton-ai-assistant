    ...
    def add_native_device(self, track_index, device_path_name):
        ...
        try:
            ...
            if not target_item:
                return {
                    "status": "error",
                    "message": f"Device {device_path_name} not found in browser.",
                }
            ...
            return {
                "status": "success",
                "message": f"Device {target_item.name} loaded on track {track_index}.",
            }
    ...
    def set_device_parameter(self, track_index, device_index, param_name, value):
        ...
        try:
            ...
            if not target_param:
                return {
                    "status": "error",
                    "message": f"Parameter {param_name} not found on device {device.name}.",
                }
            ...
            return {
                "status": "success",
                "message": f"Parameter {param_name} set to {safe_value}.",
            }
        except Exception as e:
            self.song.end_undo_step()
